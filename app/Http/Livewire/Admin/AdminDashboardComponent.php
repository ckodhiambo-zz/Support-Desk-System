    ...
    public function setSolver(Request $request)
    {
        ...
        if ($previous_solver)
        {
            if ($request->input('phone_number') == 'true') {
                ...
                $response = $at->sms($user->phone_number, 'Precision Desk - Dear ' . $user->name . ', Ticket-id #5 from: ' . $ticket->requester->name . ' of subject ' . $ticket->subject . '  has been delegated to you by ' . Auth::user()->name . ' . Kindly login to your portal for more info: https://desk.precision.ke/.');
        ...
        else
        {
            if ($ticket->requester->user_type == 'Administrator')
            {
                if ($request->input('phone_number') == 'true') {
                    ...
                    $response = $at->sms($user->phone_number, 'Precision Desk - Dear ' . $user->name . ', Ticket-id #5 from: ' . $ticket->requester->name . ' of subject ' . $ticket->subject . '  has been assigned to you by ' . Auth::user()->name . ' . Kindly login to your portal for more info: https://desk.precision.ke/.');
